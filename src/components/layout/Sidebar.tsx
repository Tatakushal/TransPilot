import { LayoutDashboard, Truck, Users, Route, Wrench, Fuel, BarChart3, Settings, Shield, Activity, X } from "lucide-react";
...
const allRoles: Role[] = ["admin", "fleet-manager", "dispatcher", "safety-officer", "financial-analyst"];
const menu: MenuSection[] = [
  { title: "Overview", items: [{ icon: LayoutDashboard, label: "Dashboard", path: "/dashboard", roles: allRoles }, { icon: Activity, label: "Operations Center", path: "/operations", roles: allRoles }] },
  ...
  { title: "Administration", items: [{ icon: Shield, label: "Admin Center", path: "/admin", roles: ["admin"] }, { icon: Settings, label: "Settings", path: "/settings", roles: ["admin"] }] },
];
...
export default function Sidebar({ mobileOpen, onClose }: Props) {
  ...
  return <>
    ...
    <aside className={`fixed inset-y-0 left-0 z-40 flex w-72 flex-col border-r border-slate-800/80 bg-slate-950 text-slate-100 shadow-2xl transition-transform duration-300 lg:static lg:z-auto lg:w-64 lg:translate-x-0 lg:shadow-none ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
      <div className="flex items-center justify-between border-b border-slate-800/80 px-5 py-5"><div className="flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-indigo-500/15 p-2 ring-1 ring-indigo-400/20"><img src="/logo.png" alt="TransPilot" className="h-full w-full object-contain" /></div><div><h1 className="text-lg font-bold tracking-tight">TransPilot</h1><p className="text-[11px] font-medium uppercase tracking-[.16em] text-slate-500">Fleet Control</p></div></div><button type="button" onClick={onClose} className="rounded-xl p-2 text-slate-400 transition hover:bg-slate-900 hover:text-white lg:hidden" aria-label="Close navigation"><X size={20}/></button></div>
