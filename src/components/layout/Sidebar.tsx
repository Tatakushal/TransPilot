import { LayoutDashboard, Truck, Users, Route, Wrench, Fuel, BarChart3, Settings, ShieldCheck, X } from "lucide-react";
...
const menu: MenuSection[] = [
  ...
  { title: "Administration", items: [{ icon: ShieldCheck, label: "Admin", path: "/admin", roles: ["admin"] }, { icon: Settings, label: "Settings", path: "/settings", roles: ["admin"] }] },
];
    ...
    <aside className={`fixed inset-y-0 left-0 z-40 flex w-72 flex-col border-r border-slate-800/80 bg-slate-950 text-slate-100 shadow-2xl transition-transform duration-300 lg:static lg:z-auto lg:w-64 lg:translate-x-0 lg:shadow-none ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
      <div className="flex items-center justify-between border-b border-slate-800/80 px-5 py-5">
        <div className="flex items-center gap-3">
          <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-indigo-500/15 p-2 ring-1 ring-indigo-400/20"><img src="/logo.png" alt="TransPilot" className="h-full w-full object-contain" /></div>
          <div><h1 className="text-lg font-bold tracking-tight">TransPilot</h1><p className="text-[11px] font-medium uppercase tracking-[.16em] text-slate-500">Fleet Control</p></div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-5">
        {menu.map((section) => { const visibleItems = section.items.filter((item) => item.roles.includes(user.role)); if (!visibleItems.length) return null; return <div key={section.title} className="mb-6"><p className="mb-2 px-3 text-[10px] font-bold uppercase tracking-[.18em] text-slate-600">{section.title}</p>{visibleItems.map((item) => { const Icon = item.icon; return <NavLink key={item.label} to={item.path} onClick={onClose} className={({ isActive }) => `group mb-1 flex items-center gap-3 rounded-xl px-3.5 py-2.5 text-sm font-medium transition-all duration-200 ${isActive ? "bg-indigo-500/15 text-indigo-300 ring-1 ring-indigo-400/15" : "text-slate-400 hover:bg-slate-900 hover:text-slate-100"}`}><Icon size={18} className="shrink-0 transition-transform group-hover:scale-105" />{item.label}</NavLink>; })}</div>; })}
      </div>
