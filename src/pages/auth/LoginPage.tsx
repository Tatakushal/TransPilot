...
import { Eye, EyeOff, LockKeyhole, Mail, ShieldCheck, Truck, UserRound } from "lucide-react";
import { useAuth } from "@/hooks/useAuth";
import type { UserRole } from "@/context/AuthContext";

const roles: Exclude<UserRole, "admin">[] = ["fleet-manager", "dispatcher", "safety-officer", "financial-analyst"];
...
export default function LoginPage() {
  ...
  async function handleSubmit() {
    ...
    try {
      if (isRegister) {
        await register(name.trim(), email.trim(), password, role as Exclude<UserRole, "admin">);
      } else {
      ...
      <div className="mx-auto grid min-h-[calc(100vh-2rem)] max-w-6xl overflow-hidden rounded-[28px] bg-white shadow-2xl lg:grid-cols-[1.05fr_.95fr]">
        ...
        <section className="flex items-center justify-center px-6 py-10 sm:px-12 lg:px-14">
          <div className="w-full max-w-md">
            <div className="mb-8 flex items-center gap-3 lg:hidden"><div className="flex h-11 w-11 rounded-xl bg-indigo-600 p-2"><img src="/logo.png" alt="TransPilot" className="h-full w-full object-contain" /></div><div><p className="font-bold">TransPilot</p><p className="text-xs text-slate-500">Fleet Operations</p></div></div>
            <div className="mb-8"><p className="mb-2 text-sm font-semibold text-indigo-600">{isRegister ? "Welcome to TransPilot 👋" : "Welcome back 👋"}</p><h2 className="text-3xl font-bold tracking-tight text-slate-900">{isRegister ? "Create your workspace" : "Sign in to your workspace"}</h2><p className="mt-2 text-sm leading-6 text-slate-500">{isRegister ? "Create an account to manage your fleet operations." : "Access your fleet command center and keep operations moving."}</p></div>

            <div className="space-y-5">
              {isRegister && <div><label htmlFor="name" className="mb-2 block text-sm font-semibold text-slate-700">Full name</label><div className="relative"><UserRound size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"/><input id="name" type="text" autoComplete="name" placeholder="Your name" value={name} onChange={e=>{setName(e.target.value);setSubmitError("");setErrors(p=>({...p,name:e.target.value.trim()?"":"Name is required"}));}} className={`h-12 w-full rounded-xl border bg-slate-50 pl-11 pr-4 text-sm outline-none transition focus:bg-white focus:ring-4 focus:ring-indigo-100 ${errors.name?"border-red-400":"border-slate-200 focus:border-indigo-500"}`}/></div>{errors.name&&<p className="mt-1.5 text-xs font-medium text-red-500">{errors.name}</p>}</div>}
              ...
              <button type="button" onClick={handleSubmit} disabled={loading} className="h-12 w-full rounded-xl bg-indigo-600 font-semibold text-white shadow-lg shadow-indigo-200 transition hover:-translate-y-0.5 hover:bg-indigo-700 hover:shadow-xl disabled:translate-y-0 disabled:cursor-not-allowed disabled:opacity-60">{loading?(isRegister?"Creating workspace...":"Signing you in..."):(isRegister?"Create account":"Sign in to dashboard")}</button>
