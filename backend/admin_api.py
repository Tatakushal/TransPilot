...
def overview(user=Depends(current_user), db: Session = Depends(get_db)):
    ...
    recent_events = db.query(AuditLogModel).count()
    return {
        ...
        "disabled_users": db.query(UserAccountModel).filter(UserAccountModel.is_active == False).count(),
        ...
        "available_drivers": db.query(DriverModel).filter(DriverModel.status == "Available").count(),
...
def users(
    ...
    return [
        {"id": r.id, "name": r.name, "email": r.email, "role": r.role, "is_active": r.is_active,
         "email_verified": r.email_verified, "created_at": r.created_at.isoformat()}
        for r in rows
    ]
...
def create_user(payload: UserCreate, user=Depends(current_user), db: Session = Depends(get_db)):
    ...
    db.add(row)
    db.flush()
    _audit(db, user, "user_created", f"user:{row.id}", f"role={row.role}")
    db.commit()
    db.refresh(row)
...
def update_user(user_id: int, payload: UserUpdate, user=Depends(current_user), db: Session = Depends(get_db)):
    ...
    if payload.email is not None and payload.email.lower() != row.email:
        if db.query(UserAccountModel).filter(UserAccountModel.email == payload.email.lower()).first():
            raise HTTPException(409, "An account with this email already exists")
    ...
    if payload.role is not None:
        if payload.role not in ROLES:
            raise HTTPException(400, "Invalid role")
        if row.id == user.id and payload.role != "admin":
    ...
    _audit(db, user, "user_updated", f"user:{row.id}", f"role={row.role},active={row.is_active}")
    db.commit()
    db.refresh(row)
    return {"id": row.id, "name": row.name, "email": row.email, "role": row.role, "is_active": row.is_active}
...
def reset_password(user_id: int, payload: PasswordUpdate, user=Depends(current_user), db: Session = Depends(get_db)):
    ...
    db.query(AuthTokenModel).filter(AuthTokenModel.user_id == row.id, AuthTokenModel.token_type == "access").update({"used": True})
    _audit(db, user, "password_reset", f"user:{row.id}", "All existing access sessions revoked")
    db.commit()
...
def revoke_session(token_id: int, user=Depends(current_user), db: Session = Depends(get_db)):
    ...
    _audit(db, user, "session_revoked", f"session:{token.id}")
    db.commit()
