...
def _apply_user_update(user_id: int, payload: UserUpdate, user, db: Session):
    _admin(user)
...
def users(search: str = Query(default="", max_length=100), role: Optional[str] = Query(default=None), status: Optional[str] = Query(default=None), user=Depends(current_user), db: Session = Depends(get_db)):
    ...
    if search.strip():
        term = f"%{search.strip().lower()}%"; query = query.filter(or_(UserAccountModel.name.ilike(term), UserAccountModel.email.ilike(term)))
    if role: query = query.filter(UserAccountModel.role == role)
    if status == "active": query = query.filter(UserAccountModel.is_active == True)
    elif status == "disabled": query = query.filter(UserAccountModel.is_active == False)
...
def update_user(user_id: int, payload: UserUpdate, user=Depends(current_user), db: Session = Depends(get_db)):
    return _apply_user_update(user_id, payload, user, db)


@router.post("/users/{user_id}/update")
def update_user_post(user_id: int, payload: UserUpdate, user=Depends(current_user), db: Session = Depends(get_db)):
    return _apply_user_update(user_id, payload, user, db)
...
def reset_password(user_id: int, payload: PasswordUpdate, user=Depends(current_user), db: Session = Depends(get_db)):
    ...
    row = db.get(UserAccountModel, user_id)
    if not row: raise HTTPException(404, "User not found")
    row.password_hash = hash_password(payload.password); db.query(AuthTokenModel).filter(AuthTokenModel.user_id == row.id, AuthTokenModel.token_type == "access").update({"used": True}); _audit(db, user, "password_reset", f"user:{row.id}", "All existing access sessions revoked"); db.commit()
    return {"message": "Password reset and existing sessions revoked"}


@router.get("/roles")
def roles(user=Depends(current_user)):
    _admin(user); return [{"id": key, "name": ROLE_LABELS[key], "permissions": ROLE_PERMISSIONS[key]} for key in ROLES]
...
def audit_logs(search: str = Query(default="", max_length=100), action: Optional[str] = Query(default=None), limit: int = Query(default=100, ge=1, le=500), user=Depends(current_user), db: Session = Depends(get_db)):
    _admin(user); query = db.query(AuditLogModel)
    if search.strip():
        term = f"%{search.strip().lower()}%"; query = query.filter(or_(AuditLogModel.user_email.ilike(term), AuditLogModel.action.ilike(term), AuditLogModel.target.ilike(term), AuditLogModel.details.ilike(term)))
...
def sessions(user=Depends(current_user), db: Session = Depends(get_db)):
    _admin(user); rows = db.query(AuthTokenModel, UserAccountModel).join(UserAccountModel, UserAccountModel.id == AuthTokenModel.user_id).filter(AuthTokenModel.token_type == "access", AuthTokenModel.used == False, AuthTokenModel.expires_at > datetime.utcnow()).order_by(AuthTokenModel.expires_at.desc()).all()
...
def revoke_session(token_id: int, user=Depends(current_user), db: Session = Depends(get_db)):
    _admin(user); token = db.query(AuthTokenModel).filter(AuthTokenModel.id == token_id, AuthTokenModel.token_type == "access").first()
    if not token: raise HTTPException(404, "Session not found")
    token.used = True; token.used_at = datetime.utcnow(); _audit(db, user, "session_revoked", f"session:{token.id}"); db.commit(); return {"message": "Session revoked"}


@router.get("/health")
def system_health(user=Depends(current_user), db: Session = Depends(get_db)):
    _admin(user); db.execute(text("SELECT 1")); return {"status": "healthy", "database": "connected", "timestamp": datetime.now(timezone.utc).isoformat()}
